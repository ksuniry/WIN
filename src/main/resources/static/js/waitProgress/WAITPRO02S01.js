...
* 수정일/내용	: 
*/
var WAITPRO02S01 = CommonPageObject.clone();

/* 화면내 변수  */
WAITPRO02S01.variable = {
	sendData		: {}								// 조회시 조건
	,detailData		: {}								// 조회 결과값
...
/* 이벤트 정의 */
WAITPRO02S01.events = {
	 'click #btnTooltip'										: 'WAITPRO02S01.event.clickBtnTooltip'			// 계좌 툴팁 아이콘 클릭시
	,'click #divTootip'											: 'WAITPRO02S01.event.clickTooltip'				// 툴팁 클릭시
	,'click #btnTopBoxButton'									: 'WAITPRO02S01.event.clickBtnTopBoxButton'		// 상단 박스내 버튼 클릭시 상태값에 따라 작업
	,'click #orgn_acnt_no_pt'									: 'WAITPRO02S01.event.clickBtnCapyAcnt'			// 계좌번호 클립보드 복사
	,'click li[id^="prodDetail_"]'								: 'WAITPRO02S01.event.goFundDetail'				// 펀드 상세 조회
	
	,'click button[id^="btnCase"]'								: 'WAITPRO02S01.event.clickMsgPopClose'			// msg 팝업 닫기
	,'click #btnRetryTrans, #btnRetryTrans2'					: 'WAITPRO02S01.event.clickBtnRetryTrans'		// 이전 재실행
}

WAITPRO02S01.init = function(){

	//여기에 최초 실행될 자바스크립트 코드를 넣어주세요
	/*eventBind*/
	CommonPageObject.eventBind('WAITPRO02S01');
	...
	WAITPRO02S01.location.pageInit();
}


// 화면내 초기화 부분
WAITPRO02S01.location.pageInit = function() {
	// 전 화면에서 받은 파라미터 셋팅
	var sParams = sStorage.getItem("WAITPRO02S01Params");
	if(!ComUtil.isNull(sParams)){
		gfn_log(sParams);
		WAITPRO02S01.variable.initParamData = sParams;
		
		WAITPRO02S01.variable.sendData.orgn_acnt_uid = sParams.orgn_acnt_uid;
		WAITPRO02S01.variable.sendData.chng_acnt_uid = sParams.chng_acnt_uid;
		WAITPRO02S01.variable.sendData.advc_gbn_cd = sParams.advc_gbn_cd;
		//sStorage.clear();
	}
	else{
		return;
	...
	//
	{
		...
		$('.dim', $('#msgPopup')).on("click",function(e){
			WAITPRO02S01.event.clickMsgPopClose(e);
		});
	}
	
	// 차트 영역 셋팅
	WAITPRO02S01.location.initChart();
	
	// 연금대기메인조회 조회  	
	WAITPRO02S01.tran.selectDetail();
...
// 거래
// 자문대기 대시보드 조회 
WAITPRO02S01.tran.selectDetail = function() {
	
	var inputParam 		= {};
	inputParam.sid 		= "pensionWaitAcntDetail";
	inputParam.target 	= "api";
	inputParam.url 		= "/pension_wait/pension_wait_acnt_detail";
	inputParam.data 	= WAITPRO02S01.variable.sendData;
	inputParam.callback	= WAITPRO02S01.callBack; 
	
	gfn_Transaction( inputParam );
}

// 자문대기 계좌상세 자문취소			 
WAITPRO02S01.tran.cancelPensionWaitAcnt = function() {
	WAITPRO02S01.variable.sendData = {};
	WAITPRO02S01.variable.sendData.acnt_uid 	= WAITPRO02S01.variable.initParamData.orgn_acnt_uid; 	// 기존계좌UID
	WAITPRO02S01.variable.sendData.advc_gbn_cd 	= WAITPRO02S01.variable.initParamData.advc_gbn_cd; 		// 자문구분코드
	WAITPRO02S01.variable.sendData.acnt_status 	= '99'; 			// 99 : 자문받지 않음 (고정값으로 셋팅)
...
// 재신청			 
WAITPRO02S01.tran.retransAcnt = function() {

	WAITPRO02S01.variable.sendData = {};
	WAITPRO02S01.variable.sendData.acnt_uid 	= WAITPRO02S01.variable.initParamData.orgn_acnt_uid; 	// 기존계좌UID
	WAITPRO02S01.variable.sendData.pwd          = WAITPRO02S01.variable.pwd								// 암호화 된 계좌 비밀번호
...
// 오픈뱅킹 인증여부 확인
WAITPRO02S01.tran.selectUserObStatus = function() {
...
// 이벤트
// 계좌 툴팁 아이콘 클릭시
WAITPRO02S01.event.clickBtnTooltip = function(e) {
 	e.preventDefault();
...
// 툴팁 클릭시
WAITPRO02S01.event.clickTooltip = function(e) {
 	e.preventDefault();
...
WAITPRO02S01.event.clickBtnTopBoxButton = function(e) {
 	e.preventDefault();

	var data = WAITPRO02S01.variable.detailData;
	...
	if("|14|".indexOf(data.acnt_status) > -1){
		...
		// 매수주문 승인하기
		WAITPRO02S01.location.callOrderPopup();
	}
}
...
// msg 팝업 닫기
WAITPRO02S01.event.clickMsgPopClose = function(e) {
 	e.preventDefault();	
...
// 문제해결 클릭시
/*WAITPRO02S01.event.clickBtnSolution = function(e) {
 	e.preventDefault();	
...
// 계좌번호복사 클릭시
WAITPRO02S01.event.clickBtnCapyAcnt = function(e) {
 	e.preventDefault();
	...
	// 클립보드 복사
	if(type == "orgn"){
		ComUtil.string.clipboardCopy(ComUtil.string.replaceAll(WAITPRO02S01.variable.detailData.orgn_acnt_no_pt, '-', ''));
	}
	else{
		ComUtil.string.clipboardCopy(ComUtil.string.replaceAll(WAITPRO02S01.variable.detailData.chng_acnt_no_pt, '-', ''));
	}
...
// '이전 재실행' 클릭시
WAITPRO02S01.event.clickBtnRetryTrans = function(e) {
 	e.preventDefault();

	gfn_confirmMsgBox("계좌이전을 재실행 하시겠습니까?", '', function(returnData){
		if(returnData.result == 'Y'){

			WAITPRO02S01.location.modalClose();
			
			$('#msgPopup').show();
			
			var inputParam = {};
			inputParam.callback = WAITPRO02S01.callBack.popPassword;
			inputParam.pwdMsg = "계좌개설 시 설정한<br>비밀번호를 입력해주세요";
			gfn_callPwdPopup(inputParam);
			
			//WAITPRO02S01.event.kbKeyPadInit();
			//$('#msgCase_pwd').show();
		}
	});
}
...
// 해지신청  후 입금 클릭시
/*WAITPRO02S01.event.clickBtnTerminate = function(e) {
 	e.preventDefault();
	...
	gfn_confirmMsgBox("해지신청  후 입금.", '', function(returnData){
		...
		if(returnData.result == 'Y'){
			// 오픈뱅킹 인증여부 확인
			WAITPRO02S01.tran.selectUserObStatus();
		}
		else{
...
// 자문대기 계좌상세 자문취소 클릭시
/*WAITPRO02S01.event.changeCancelAcnt = function(e) {
 	e.preventDefault();
	...
	gfn_confirmMsgBox("더이상 연금관리를 받지 않을실건가요? 멘트주세요.", '', function(returnData){
		...
		if(returnData.result == 'Y'){
			WAITPRO02S01.tran.cancelPensionWaitAcnt();
		}
		else{
			$(this).attr( "checked", false );
		}
...
// 펀드상세 외부 링크 화면 호출 
WAITPRO02S01.event.goFundDetail = function(e) {
	e.preventDefault();
...
////////////////////////////////////////////////////////////////////////////////////
// 콜백 함수
WAITPRO02S01.callBack = function(sid, result, success){
	...
	if(sid == "pensionWaitAcntDetail"){
		...
		WAITPRO02S01.variable.detailData = result;
		
		WAITPRO02S01.variable.detailData.chng_acnt_nm = WAITPRO02S01.variable.initParamData.chng_acnt_nm;	// 이전 화면의 데이터를 이용한다.
		
		// 상세 셋팅 
		WAITPRO02S01.location.displayDetail();
		return;
	}
	// 계좌이전 재신청
...
// 네이티브 호출후 콜백함수 
WAITPRO02S01.callBack.native = function(result){
	var key = result.key;
...
// 비밀번호 입력후 팝업 콜백함수 
WAITPRO02S01.callBack.popPassword = function(returnParam){
	...
	if(!ComUtil.isNull(returnParam.pwd)){
		...
		WAITPRO02S01.variable.pwd = returnParam.pwd;

		WAITPRO02S01.tran.retransAcnt();
...
// 상세 셋팅
WAITPRO02S01.location.displayDetail = function(){
	var detailData = WAITPRO02S01.variable.detailData;
	
	//debugger;
	
	// 추가적인 정보 셋팅
	$.extend(detailData, WAITPRO02S01.location.setInfoByStatus(detailData));
	...
	// chart
	WAITPRO02S01.location.displayChart();
...
WAITPRO02S01.location.setInfoByStatus = function(itemInfo){
	//itemInfo.acnt_status = '25';
...
// order popup call
WAITPRO02S01.location.callOrderPopup = function(){
	var sParam = {};
...
// chart
WAITPRO02S01.location.displayChart = function(){
	var detailData = WAITPRO02S01.variable.detailData;
...
// 차트 그리기
// 차트 초기
WAITPRO02S01.location.initChart = function(){
...
//'매수 주문 승인 하기 Popup'
WAITPRO02S01.event.clickBuyApproval = function () {
...
// modal popup close
WAITPRO02S01.location.modalClose = function(){
