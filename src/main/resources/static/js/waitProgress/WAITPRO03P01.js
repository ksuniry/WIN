...
* 수정일/내용	: 
*/
var WAITPRO03P01 = CommonPageObject.clone();

/* 화면내 변수  */
WAITPRO03P01.variable = {
	sendData		: {}								// 조회시 조건
	,detailData		: {}								// 조회 결과값
...
/* 이벤트 정의 */
WAITPRO03P01.events = {
	 'click a[id^="btnAdviceDetail_"]'						: 'WAITPRO03P01.event.clickBtnAdviceDetail'		// 자문안보기 버튼 클릭시
	,'click i[id^="btnTooltip_"]'							: 'WAITPRO03P01.event.clickBtnTooltip'			// 계좌 툴팁 아이콘 클릭시
	,'click div[id^="tootip_"]'								: 'WAITPRO03P01.event.clickTooltip'				// 툴팁 클릭시
	,'click button[id^="btnBox_"]'							: 'WAITPRO03P01.event.clickBtnBox'				// 박스네 버튼 클릭시
}

WAITPRO03P01.init = function(){

	//여기에 최초 실행될 자바스크립트 코드를 넣어주세요
	/*eventBind*/
	CommonPageObject.eventBind('WAITPRO03P01');
	...
	WAITPRO03P01.location.pageInit();
}


// 화면내 초기화 부분
WAITPRO03P01.location.pageInit = function() {
	
	// 월적립식 투자 상세 조회
	WAITPRO03P01.tran.selectDetail();
...
// 거래
// 월적립식 투자 상세 조회 
WAITPRO03P01.tran.selectDetail = function() {
	
	var inputParam 		= {};
	inputParam.sid 		= "getMonPayExecuteList";
	inputParam.target 	= "api";
	inputParam.url 		= "/pension_wait/get_mon_pay_execute_list";
	inputParam.data 	= WAITPRO03P01.variable.sendData;
	inputParam.callback	= WAITPRO03P01.callBack;
...
// 이벤트
// 자문안보기 버튼 클릭시
WAITPRO03P01.event.clickBtnAdviceDetail = function(e) {
 	e.preventDefault();

	var idx = $(this).data('idx');
	var data = $('#acntDetail_' + idx).data();
	
	sStorage.setItem("WAITPRO04P01Params", data);

	//var url = "/wait_progress/WAITPRO04S01";
	//ComUtil.moveLink(url);
	var sParam = {};
	sParam.url = '/wait_progress/WAITPRO04P01'; // 월 적립식 투자 화면으로 이동
	gfn_callPopup(sParam, function(){});
}


// 계좌 툴팁 아이콘 클릭시
WAITPRO03P01.event.clickBtnTooltip = function(e) {
 	e.preventDefault();
...
// 툴팁 클릭시
WAITPRO03P01.event.clickTooltip = function(e) {
 	e.preventDefault();
...
// 툴팁 클릭시
WAITPRO03P01.event.clickBtnBox = function(e) {
 	e.preventDefault();
...
////////////////////////////////////////////////////////////////////////////////////
// 콜백 함수
WAITPRO03P01.callBack = function(sid, result, success){
	if(success != "success" ) {
	...
	if(sid == "getMonPayExecuteList"){
		...
		WAITPRO03P01.variable.detailData = result;
		// advc_exe_status ( 1 : 실행중 / 2 :실행완료 )
		WAITPRO03P01.variable.detailData.advc_exe_status_msg = (result.advc_exe_status == '1' ? "실행중" : "실행완료");
		
		// 상세 셋팅 
		WAITPRO03P01.location.displayDetail();
		return;
	}
}
...
// 상세 셋팅
WAITPRO03P01.location.displayDetail = function(){
	var detailData = WAITPRO03P01.variable.detailData;
	...
	$.each( detailData.mon_invt_list, function(index, item){
		...
		// 추가적인 정보 셋팅
		$.extend(item, WAITPRO03P01.location.setInfoByStatus(item));
	...
	// 상세내역 셋팅
	gfn_setDetails(detailData, $('#w0301-content'));
...
*/
WAITPRO03P01.location.setInfoByStatus = function(itemInfo){
	var addInfo = {};
	switch(itemInfo.acnt_status){
		...
		case '26'	: 
						...
						addInfo.beforeImg 	= "order.png";
						addInfo.afterImgSrc = gfn_getImgSrcByCd(itemInfo.kftc_agc_cd, 'G');
						addInfo.mBoxMsg		= "해지후 입금·포트폴리오 매수가 일부만 실행되었습니다.<br>잔여분 실행을 위해서 매수주문을 승인해 주세요.";
						addInfo.mBtnTitle	= "매수주문 승인하기";
			break;
		case '29'	: 
						addInfo.mTitle 		= "입금하기";
