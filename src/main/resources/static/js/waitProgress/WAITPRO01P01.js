...
* 수정일/내용	: 
*/
var WAITPRO01P01 = CommonPageObject.clone();

/* 화면내 변수  */
WAITPRO01P01.variable = {
	sendData		: {}								// 조회시 조건
	,detailData		: {}								// 조회 결과값
...
/* 이벤트 정의 */
WAITPRO01P01.events = {
	 'click #showIcoTrans'									: 'WAITPRO01P01.event.clickShowIcoTrans'
	,'click li[id^="unexec_"]'								: 'WAITPRO01P01.event.clickMoveDetail'
	,'click .underline'										: 'WAITPRO01P01.event.clickBtnCapyAcnt'
	
	,'click i[id^="btnCallPopup_"]'							: 'WAITPRO01P01.event.clickBtnCallPopup'		// 내부 설명팝업 호출 
	,'click button[id^="btnBox_"]'							: 'WAITPRO01P01.event.clickBtnBox'				// 박스내 버튼 클릭시 (계좌이전 재신청 등.) 
	,'click i[id^="btnTooltip_"]'							: 'WAITPRO01P01.event.clickBtnTooltip'			// 계좌 툴팁 아이콘 클릭시
	,'click div[id^="tootip_"]'								: 'WAITPRO01P01.event.clickTooltip'				// 툴팁 클릭시
	
	,'click #pop2Close'										: 'WAITPRO01P01.event.clickPop2Close'
	,'click #pop3Close'										: 'WAITPRO01P01.event.clickPop3Close'
	,'click #btnRetryTrans'									: 'WAITPRO01P01.event.clickBtnRetryTrans'		// 이전 재실행
	,'click a[id^="btnAdviceDetail_"]'						: 'WAITPRO01P01.event.clickBtnAdviceDetail'		// 자문안보기 버튼 클릭시
}

WAITPRO01P01.init = function(){

	//여기에 최초 실행될 자바스크립트 코드를 넣어주세요
	/*eventBind*/
	CommonPageObject.eventBind('WAITPRO01P01');
	...
	WAITPRO01P01.location.pageInit();
}


// 화면내 초기화 부분
WAITPRO01P01.location.pageInit = function() {	

	{
		$('.dim', $('#w0101Pop2')).on("click", WAITPRO01P01.event.clickPop2Close);
	}
	
	// 연금대기메인조회 조회
	WAITPRO01P01.tran.selectDetail();
}
...
// 거래
// 자문대기 대시보드 조회 
WAITPRO01P01.tran.selectDetail = function() {
	...
	inputParam.data 	= {};
	inputParam.callback	= WAITPRO01P01.callBack; 
	
	gfn_Transaction( inputParam );
}

// '계좌 이전 재신청'			 
WAITPRO01P01.tran.retransAcnt = function() {
	
	WAITPRO01P01.variable.sendData = {};
	WAITPRO01P01.variable.sendData.acnt_uid 	= WAITPRO01P01.variable.detailData.orgn_acnt_uid; 		// 기존계좌UID
	WAITPRO01P01.variable.sendData.pwd          = WAITPRO01P01.variable.pwd								// 암호화 된 계좌 비밀번호

	var inputParam 		= {};
	inputParam.sid 		= "pensionWaitAcntReTrans";
	inputParam.target 	= "api";
	inputParam.url 		= "/pension_wait/pension_wait_acnt_re_trans";
	inputParam.data 	= WAITPRO01P01.variable.sendData;
	inputParam.callback	= WAITPRO01P01.callBack;
...
// 툴팁보기 클릭시
WAITPRO01P01.event.clickShowIcoTrans = function(e) {
 	e.preventDefault();

	gfn_log($(this).data('type'));
	
	$('#noteTrans', $('#w0101-content')).addClass('show');
    /*
	setTimeout(function(){ 
...
// 상세화면이동 클릭시
WAITPRO01P01.event.clickMoveDetail = function(e) {
 	e.preventDefault();

	var data = $(this).data();
	// 2: 이체, 9:해지 5:신규
	if(data.advc_gbn_cd == '5'){
		sStorage.setItem("WAITPRO02P06Params", data);
		
		//ComUtil.moveLink('/wait_progress/WAITPRO02S06'); //신규상세 화면이동
		
		var sParam = {};
		sParam.url = '/wait_progress/WAITPRO02P06'; //신규상세 화면이동
		gfn_callPopup(sParam, function(){});
	}
	else{
		sStorage.setItem("WAITPRO02P01Params", data);
		//ComUtil.moveLink('/wait_progress/WAITPRO02S01'); // 상세 화면이동
		var sParam = {};
		sParam.url = '/wait_progress/WAITPRO02P01'; //상세 화면이동
		gfn_callPopup(sParam, function(){});
	}
}
...
//계좌 번호 복사 하기
WAITPRO01P01.event.clickBtnCapyAcnt = function() {
...
// 내부 설명팝업 호출
WAITPRO01P01.event.clickBtnCallPopup = function() {
	
	var idx = $(this).data('idx');
	var data = $('#unexec_' + idx, $('#w0101-content')).data();
	...
	if("|14|".indexOf(data.acnt_status) > -1){
		...
		//'해지 후 입금' Popup 호출
		var sParam = {};
		sParam.url = '/wait_progress/WAITPRO01P01P01';
...
WAITPRO01P01.event.clickBtnBox = function() {
	...
	if("|15|".indexOf(data.acnt_status) > -1){
		...
		data.orgn_acnt_no_pt 		= ComUtil.pettern.acntNo(ComUtil.null(data.orgn_acnt_no, '') + gfn_getAddAcntNoByCd(data.orgn_kftc_agc_cd));
		WAITPRO01P01.variable.detailData.orgn_acnt_uid = data.orgn_acnt_uid;	// 이전 신청할 uid 
		...
		// 매수주문 승인하기
		WAITPRO01P01.location.callOrderPopup();
	}
}

//'유의사항' 모달창 닫기
WAITPRO01P01.event.clickPop2Close = function() {
	$("#w0101Pop2").hide();
}

//'계좌이전 재신청' 모달창 닫기
WAITPRO01P01.event.clickPop3Close = function() {
	$("#w0101Pop3").hide();
}


// 계좌 툴팁 아이콘 클릭시
WAITPRO01P01.event.clickBtnTooltip = function(e) {
 	e.preventDefault();
...
// 툴팁 클릭시
WAITPRO01P01.event.clickTooltip = function(e) {
 	e.preventDefault();
...
// '이전 재실행' 클릭시
WAITPRO01P01.event.clickBtnRetryTrans = function(e) {
 	e.preventDefault();

	gfn_confirmMsgBox("계좌이전을 재실행 하시겠습니까?", '', function(returnData){
		if(returnData.result == 'Y'){

			WAITPRO01P01.location.modalClose();			 
			
			$('#msgPopup', $('#w0101-content')).show();
			
			var inputParam = {};
			inputParam.callback = WAITPRO01P01.callBack.popPassword;
...
// 자문안보기 버튼 클릭시
WAITPRO01P01.event.clickBtnAdviceDetail = function(e) {
 	e.preventDefault();
	...
	var sParam = {};
	sParam.url = '/wait_progress/WAITPRO02P01';
	gfn_callPopup(sParam, function(){});
}
...
////////////////////////////////////////////////////////////////////////////////////
// 콜백 함수
WAITPRO01P01.callBack = function(sid, result, success){
	...
	if(sid == "pensionWaitMain"){
		WAITPRO01P01.variable.detailData = result;
		// advc_exe_status ( 1 : 실행중 / 2 :실행완료 )
		WAITPRO01P01.variable.detailData.advc_exe_status_msg = (result.advc_exe_status == '1' ? "실행중" : "실행완료");
		
		WAITPRO01P01.variable.detailData.user_nm = JsEncrptObject.decrypt(WAITPRO01P01.variable.detailData.user_nm);
		if(ComUtil.isNull(WAITPRO01P01.variable.detailData.user_nm)){
			WAITPRO01P01.variable.detailData.user_nm = gfn_getUserInfo('userNm', true);
		}
		
		// 상세 셋팅 
		WAITPRO01P01.location.displayDetail();
		
		// order popup call
		/*if(WAITPRO01P01.variable.showOrder){
			WAITPRO01P01.location.callOrderPopup();
		}*/
	}
	// 계좌이전 재신청
...
// 비밀번호 입력후 팝업 콜백함수 
WAITPRO01P01.callBack.popPassword = function(returnParam){
	if(ComUtil.isNull(returnParam)){
		return;
	}
	
	if(!ComUtil.isNull(returnParam.pwd)){
		var cnfrCd = $('#nintv_sm_cnfr_cd', $('#w0101-content')).val();
		
		//KB키보드 입력을 통해 입력 한 패스워드 암호화 되어 넘어온 데이터 셋팅
		WAITPRO01P01.variable.pwd = returnParam.pwd;

		WAITPRO01P01.tran.retransAcnt();
		
		//$("#msgCase_pwd").hide();
		$('#msgPopup', $('#w0101-content')).hide();
	}
}
...
// 상세 셋팅
WAITPRO01P01.location.displayDetail = function(){
	var detailData = WAITPRO01P01.variable.detailData;
	
	// 초기화
	$('#ulUnexecList', $('#w0101-content')).html('');
	...
	// 상세내역 셋팅
	gfn_setDetails(detailData, $('#w0101-content'));
	...
	$.each(detailData.unexec_list, function(index, item){
		...
		// 상태태그
		//item.stateHtml = WAITPRO01P01.location.makeStateHtml(item.status_tag);
		// 계좌번호 패턴 추가
		item.acnt_no_pt = ComUtil.pettern.acntNo(item.acnt_no + gfn_getAddAcntNoByCd(item.kftc_agc_cd));
		
		// 추가적인 정보 셋팅
		//item.acnt_status = '15';
		$.extend(item, WAITPRO01P01.location.setInfoByStatus(item));
		
		item.status_title = item.title;
		
		var html = template(item);
		$('#ulUnexecList', $('#w0101-content')).append(html);
		
		$('#unexec_' + item.idx, $('#w0101-content')).data(item);
	});
...
var arr2 = ['03','04','16','19','24','27'];			//금융 아이콘3개	

case '03' 	: titleNm = '상품 매수 진행중';
case '04' 	: titleNm = '매수주문실패';
...
*/
WAITPRO01P01.location.setInfoByStatus = function(itemInfo){
	var addInfo = {};
...
// 상태태그 만들기
WAITPRO01P01.location.makeStateHtml = function(status_tag){
...
// order popup call
WAITPRO01P01.location.callOrderPopup = function(){
	
	// 무조건 주문창을 띄운다.
	//if(  ComUtil.null(WAITPRO01P01.variable.detailData.buy_order_yn, 'N') == 'Y'
	//  || ComUtil.null(WAITPRO01P01.variable.detailData.sell_order_yn, 'N') == 'Y'
	//  )
    {
		var sParam = {};
		...
		gfn_callPopup(sParam, function(){
			// 화면엔 한번만 주문창을 호출한다. 
			//WAITPRO01P01.variable.showOrder = false;
			// 연금대기메인조회 조회  	
			//WAITPRO01P01.tran.selectDetail();
		});
	}
...
// modal popup close
WAITPRO01P01.location.modalClose = function(){
    $('#w0101Pop3').hide();
}
