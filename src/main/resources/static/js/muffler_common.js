...
function gfn_setProtectPage(screenId){
	...
	if(ComUtil.isNull(sStorage.getItem('g_protectPage'))){
		...
		g_protectPage.push('/untact_open/UNTOPEN01S01');
...
function gfn_checkSystem(screenId){
	// 시스템 체크 제외  화면정의 
	if("CMMSYSE01P01|TEMPLATE|TESTMAIN|MYINFOM05S01|".indexOf(screenId) > -1 ){
		return;
	}
	var inputParam 		= {};
	inputParam.sid 		= "checkSystemStatus";
	inputParam.target 	= "api";
	inputParam.url 		= "/pension_advice/check_system_status";
	inputParam.data 	= {};
...
*/
	if(success != "success" ) {
		//gfn_callPopupSysClose({msg:"마이머플러 서비스를 현재 사용하실수 없습니다."});
		gfn_callPopupSysClose({msg:mesg});
		//gfn_callPopupSysClose();
		return;
	}
	
	// 전체 상태
	if(result.total_status == 'Y'){
		gfn_callPopupSysClose({msg:result.total_status_msg});
	}
	
	// 스크래핑 상태
	if(result.scrapping_status == 'Y'){
		//gfn_callPopupSysClose({msg:result.scrapping_status_msg});
		//gfn_callPopupSysClose();
	}
	
	// 연금자문 상태
	if(result.pension_advice_status == 'Y'){
		if(location.pathname.indexOf('/dashBoard') > -1
		){
			gfn_callPopupSysClose({msg:result.pension_advice_status_msg});
		}
	}
	
	// 계좌계설 상태
	if(result.account_open_status == 'Y'){
		if(location.pathname.indexOf('/untact_open') > -1
		|| location.pathname.indexOf('/pension_execution/PENSEXE') > -1
		){
			gfn_callPopupSysClose({msg:result.account_open_status_msg});
		}
	}
	
	// 연금관리 상태
	if(result.pension_mng_status == 'Y'){
		if(location.pathname.indexOf('/pension_mng') > -1
		){
			gfn_callPopupSysClose({msg:result.pension_mng_status_msg});
		}
	}
	
	// 미성년자 사용자
	if(result.minor_user_status == 'Y'){
		gfn_callPopupSysClose({msg:result.minor_user_status_msg});
	}
	
	// 은퇴후 사용자
	if(result.older_user_status == 'Y'){
		gfn_callPopupSysClose({msg:result.older_user_status_msg});
	}
...
function gfn_getOprtStatusInfo(oprt_status_cd){
	...
	switch(sdc){
		...
		case '4' : oprtInfo.operStatusText 	= "잠김";
		           ...
		           oprtInfo.bHelpBtn		= true;
			break; 
		case '99' : oprtInfo.operStatusText 	= "자문 전";
...
function gfn_callBackSetUInfoInit(sid, result, success){
	...
	gfn_setUserInfo('status', 		result.status);		// 'advice' : 자문전, 'trans' : 자문계약중, 'mng' : 자문완료
	gfn_setHeaderInfo();
}
